...
#   Схема Додатка 4: 3 блоки за ПЕРШОЮ літерою — Блок1 [А,К,Т,М], Блок2 [В,Н,Х,Е], Блок3 [С,І,О,Р];
#   ДРУГА літера в межах блоку ідентифікує регіон. Тобто коди регіону = {літери блоку} × {літера регіону}.
#   Перевірено на реальній базі: усі 36 наявних префіксів збігаються з цією моделлю (0 розбіжностей).
REGION_SERIES: dict = {
    "м. Київ": ["АА", "КА", "ТА", "МА"],
    "Вінницька": ["АВ", "КВ", "ТВ", "МВ"],
    "Волинська": ["АС", "КС", "ТС", "МС"],
    "Дніпропетровська": ["АЕ", "КЕ", "ТЕ", "МЕ"],
    "Донецька": ["АН", "КН", "ТН", "МН"],
    "Житомирська": ["АМ", "КМ", "ТМ", "ММ"],
    "Закарпатська": ["АО", "КО", "ТО", "МО"],
    "Запорізька": ["АР", "КР", "ТР", "МР"],
    "Івано-Франківська": ["АТ", "КТ", "ТТ", "МТ"],
    "Київська": ["АІ", "КІ", "ТІ", "МІ"],
    "Харківська": ["АХ", "КХ", "ТХ", "МХ"],
    "АР Крим": ["АК", "КК", "ТК", "МК"],
    "Кіровоградська": ["ВА", "НА", "ХА", "ЕА"],
    "Львівська": ["ВС", "НС", "ХС", "ЕС"],
    "Миколаївська": ["ВЕ", "НЕ", "ХЕ", "ЕЕ"],
    "Одеська": ["ВН", "НН", "ХН", "ЕН"],
    ...
    "Хмельницька": ["ВХ", "НХ", "ХХ", "ЕХ"],
    "Луганська": ["ВВ", "НВ", "ХВ", "ЕВ"],
...
async def show(bot: Bot, chat_id: int, text: str, kb: InlineKeyboardMarkup) -> None:
    ...
    if mid is not None:
        try:
            ...
            # Identical content → keep the current screen, don't spawn a duplicate.
