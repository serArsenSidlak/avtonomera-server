...
# Коди регіонів (перші 2 літери) — повна офіційна таблиця (Додаток 4), 4 коди на регіон:
# активні (нові) першими, далі легасі/резервні. Звірено по 20+ млн реальних номерів —
# 108 кодів, без дублів і конфліктів регіонів.
REGION_SERIES: dict = {
    "АР Крим": ["АК", "МА", "ТК", "МК"],
    "Вінницька": ["АВ", "КВ", "ММ", "ОК"],
    ...
    "Київська": ["АІ", "КІ", "ТІ", "МЕ"],
    "м. Київ": ["АА", "КА", "ТТ", "ТА"],
    "Кіровоградська": ["ВА", "НА", "ХА", "ЕА"],
    "Луганська": ["ВВ", "НВ", "ЕЕ", "ЕВ"],
...
async def cb_cmb_free(cq: CallbackQuery, state: FSMContext) -> None:
    """Крок «вибір серії»: усі літеросполуки обраного регіону (навіть ще не зустрічались)."""
    await cq.answer()
    cf = await _cf(state)
    region = cf.get("region")
    if not region:  # серія = код регіону → спершу обери регіон
    ...
    rows = [2] * (len(codes) // 2) + ([1] if len(codes) % 2 else [])
    b.button(text="⌨️ Інша серія", callback_data="cmb:psother")
    b.button(text="🌍 Інший регіон", callback_data="cmb:setreg")
    b.button(text="⬅️ Розділи", callback_data="cmb:cats")
    rows += [2, 1]
    b.adjust(*rows)
    await show(cq.message.bot, cq.message.chat.id,
              f"⚪ <b>Вільні</b> · {cf.get('digits')}\n━━━━━━━━━━━━\n<b>{region}</b>\n\n"
              "Обери <b>серію</b> (літеросполуку регіону) — покажу всі вільні номери цієї серії "
              "(навіть ті, що ще не зустрічались):", b.as_markup())
